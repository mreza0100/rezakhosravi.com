...
- Essays are generated: edit the markdown sources, then run `python3 ~/work/host-ops/blog/infra/migrate.py` from this directory. It writes `essays/<slug>/index.html` and `index.md`, `img/essays/`, the essays index, the homepage list, `feed.xml`, `sitemap.xml`, `llms.txt` and `llms-full.txt`. The reading order and numbers live in `~/work/host-ops/tmp/SERIES`; alt text in `~/work/host-ops/tmp/ALT.tsv`.
