...
## What a miracle would be

A miracle is an event that happens without a cause or explanation, defying the laws of nature. We might see them as randomness, but randomness differs from unpredictability: randomness means no cause exists, while unpredictability means we don’t know the cause.

In the world described in the previous articles, where everything follows from necessity, miracles have no place.
...
- **Not contingent, but conditioned:** Nothing is free in the world. Everything is conditioned by the necessity of the divine nature and can only follow its predetermined path.
- **Necessity of the divine nature:** Like the triangle example, divine nature represents the unbreakable rules of nature due to their perfect nature.

> *[E1p33](https://ethica.bc.edu/#/element/1P33) “Things could have been produced by God (nature) in no other way, and in no other order than that in which they have been produced.”*

Things could only happen the way they did. Not because someone chose it, but because the laws of nature made it necessary.

### Every moment is a node

Assume every moment is a node in a linear graph:

1 → 2 → 3 → 4 → 5 → 6 → … → 1000

The result of the next moment follows from the previous moment, and so on. The sequence is fixed: the future results from the past.
...
Miracles are events without a cause, beyond the laws of nature. It is impossible to break the perfect laws of nature because they are part of reality, the very same way the triangle is part of reality.

Everything happens because of necessity, with infinite accuracy. There is no room for miracles in the chain of cause and effect.
